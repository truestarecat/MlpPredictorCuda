...
namespace mlp_network
{
	...
	private:
		...
		/// </summary>
		double divideFactor_;
		double maxLearningRms_;
		size_t numInput_;
		size_t numHidden_;
		size_t numOutput_;
		size_t maxNumEpoch_;
		...
		double momentum_;
		size_t hiddenFunctionIndex_;
		size_t outputsFunctionIndex_;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->numHiddenNumericUpDown->Value = System::Decimal(gcnew cli::array< System::Int32 >(4) { 4, 0, 0, 0 });
			// 
			// label1
			// 
			...
			this->numInputNumericUpDown->Value = System::Decimal(gcnew cli::array< System::Int32 >(4) { 7, 0, 0, 0 });
			// 
			// label4
			// 
			...
			this->maxNumIterationNumericUpDown->Increment = System::Decimal(gcnew cli::array< System::Int32 >(4) { 50, 0, 0, 0 });
			this->maxNumIterationNumericUpDown->Location = System::Drawing::Point(208, 65);
			this->maxNumIterationNumericUpDown->Maximum = System::Decimal(gcnew cli::array< System::Int32 >(4) { 10000, 0, 0, 0 });
			this->maxNumIterationNumericUpDown->Minimum = System::Decimal(gcnew cli::array< System::Int32 >(4) { 1, 0, 0, 0 });
			...
			this->maxNumIterationNumericUpDown->Value = System::Decimal(gcnew cli::array< System::Int32 >(4) { 500, 0, 0, 0 });
			// 
			// divideFactorTextBox
			// 
			...
			this->learningRateTextBox->Text = L"0,15";
			// 
			// momentumTextBox
			// 
			...
			this->momentumTextBox->Text = L"0,75";
			// 
			...
			this->tabPage2->Size = System::Drawing::Size(597, 502);
			...
			this->errorGraph->Size = System::Drawing::Size(591, 496);
...
#pragma endregion
	...
	{
		divideFactor_ = Convert::ToDouble(divideFactorTextBox->Text);
		maxLearningRms_ = Convert::ToDouble(maxLearningRmsTextBox->Text);
		...
		learningRate_ = Convert::ToDouble(learningRateTextBox->Text);
		momentum_ = Convert::ToDouble(momentumTextBox->Text);
	...
	private: System::Void LearnNetwork()
	{
		...
		MlpNetwork network(numInput_, numHidden_, numOutput_,
			ActivationFunction(hiddenFunctionType), ActivationFunction(outputsFunctionType));

		vector<double> rawInputData = NetworkPrediction::loadData("InputData.txt");
		...
		{
			double tempLearningRate = learningRate_;
			double tempMomentum = momentum_;
		...
		vector<double> errors = training->train(maxNumEpoch_, maxLearningRms_);

		numEpochLabel->Text = Convert::ToString(training->numEpoch());
		learningRmsLabel->Text = Convert::ToString(training->error());

		const matrix<double> predictedOutputMatrix = network.computeOutputs(fullDataset.inputData());
		//const matrix<double> predictedOutputMatrix = network.computeOutputs(testingDataset.inputData());

		const vector<double> predictedOutputVector = MatrixHelper::convertMatrixToVector(predictedOutputMatrix);

		const vector<double> testingOutputVector = MatrixHelper::convertMatrixToVector(fullDataset.outputData());
		//const vector<double> testingOutputVector = MatrixHelper::convertMatrixToVector(testingDataset.outputData());

		testingRmsLabel->Text = Convert::ToString(MatrixHelper::rms(predictedOutputVector, testingOutputVector));

		DrawGraph(resultGraph, gcnew array<String ^> { L"Исходный график", L"Нейронная сеть" },
			gcnew array<String ^> { L"Red", L"Blue"}, matrix<double> { testingOutputVector, predictedOutputVector });

		DrawGraph(errorGraph, gcnew array<String ^> { L"" }, gcnew array<String ^> { L"Red" }, matrix<double> { errors });
	}
